        ...
        if(option?.function){
            ...
            if(option?.isValidation){
                ...
                if(result?.error){
                    ...
                    /*if(socket){
                        //socket.emit(listen+option?.path,SocketResponse.error(response))
                ...
                data=data?.data
                if(typeof option?.args!='object') option.args={}
                option.args.acebaseClient=req?.acebaseClient
                ...
                if(result?.error|| result?.data===false){
                    response.error={}
                    response.error.auth=result.error
                    /*if(socket){
                        socket.emit(listen+path,SocketResponse.error(response))
                    }*/
                    return Response.error(res,response,response.error.auth?.code)
                }
